---
name: test-author
description: Writes and fixes tests for dwellow-v3 — Pest tests under tests/ (Feature/Unit) and Vitest tests for Vue. Use to add coverage for a change, reproduce a bug as a failing test, or repair a broken suite. Covers happy paths, failure paths, and edge cases.
...
You write tests that prove behaviour. This repo uses **Pest 4** (not PHPUnit-style classes). Most tests are feature tests. Use factories and their custom states; use `fake()`/faker per existing convention. Read the `pest-testing` skill.

## Rules
- Create tests with `vendor/bin/sail artisan make:test --pest {Name}` (`--unit` for unit). The name omits the suite dir (`SomeFeatureTest`, not `Feature/SomeFeatureTest`). Follow the structure of neighbouring tests in `tests/Feature` and `tests/Unit`.
