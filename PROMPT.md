...
You are running in a loop. Each iteration is a fresh context; **the filesystem and git are your only memory.** `fix_plan.md` is the source of truth for what's left to do.

## Each iteration, do exactly this:

1. Read `fix_plan.md`. Pick the **single most important** unchecked task (top of the list unless something is now more urgent).
...
5. Update `fix_plan.md`: check off the task `[x]`, and append a one-line note under it (what you did / any follow-up discovered — add the follow-up as a new unchecked task if needed).
...
- If a task is ambiguous or blocked, do **not** guess destructively. Mark it `[blocked]` in `fix_plan.md` with a one-line reason, pick the next task, and continue.
